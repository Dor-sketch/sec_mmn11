...
User::User()
{
    throw std::runtime_error("User constructor should not be called without a USocial pointer");
}

User::User(std::shared_ptr<USocial> us)
{
    this->us = us;
}
...
User::~User() {
    ...
    for (std::list<std::shared_ptr<Message>>::iterator it = this->receivedMsgs.begin();
...
std::list<std::shared_ptr<Post>> User::getPosts() const
{
    return this->posts;
}
...
void User::viewFriendsPosts() const
{
    ...
    if (!usocial)
    { // Check if the resulting shared_ptr is valid
        throw std::runtime_error("Associated USocial object is no longer available.");
    }

    for (std::list<unsigned long>::const_iterator it = this->friends.begin();
         it != this->friends.end(); ++it)
    {
        std::shared_ptr<User> u = usocial->getUserById(*it); // Use the locked shared_ptr
...
void User::sendMessage(std::shared_ptr<User> u, std::shared_ptr<Message> m)
{
    ...
    if (this->friends.empty())
        throw std::runtime_error("Sending failed. " + this->getName() + " has no friends to send messages to.");

    for (std::list<unsigned long>::const_iterator it = this->friends.begin();
         it != this->friends.end(); ++it)
    {
        if (usocial->getUserById(*it) == usocial->getUserById(u->getId()))
        { // Use the locked shared_ptr
            u->receiveMessage(m);
            return;
        }
    }
    throw std::runtime_error(u->getName() + " is not a friend of " + this->getName() + ". Regular user can only message his friends.");
}

// business user can send messages to any existing user
void BusinessUser::sendMessage(std::shared_ptr<User> u, std::shared_ptr<Message> m)
{
    std::shared_ptr<USocial> usocial = us.lock(); // Lock the weak_ptr
    if (!usocial)
    { // Check if the resulting shared_ptr is valid
    ...
    if (usocial->getUserById(u->getId()) == NULL)
    { // Use the locked shared_ptr
        throw std::runtime_error("Sending failed. Recipient " + u->getName() + " does not exist.");
    }
    u->receiveMessage(m);
}
...
void User::viewReceivedMessages()
{
    try
    {
        for (std::list<std::shared_ptr<Message>>::iterator it = this->receivedMsgs.begin(); it != this->receivedMsgs.end(); ++it)
        {
            if ((*it)->getText().empty())
